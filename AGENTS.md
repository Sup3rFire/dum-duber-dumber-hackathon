...
- `extension/manifest.json` permissions — new entries in `permissions`,
...
- New `storage.local`/`storage.sync` fields beyond settings, API keys,
  ...
  (`memCache`, `countedSessions` in `extension/background.js`).
- `storage.sync` instead of `storage.local`.
- Injection running on a site the user hasn't enabled.
- Unvendored third-party/CDN scripts (`extension/lib/browser-polyfill.js` is
  vendored on purpose).
- New required (vs. optional, gesture-requested) permissions.

UI, `extension/prompt.js`, `extension/providers.js`, or in-memory-only caching
changes don't need this — default to flagging when unsure.
